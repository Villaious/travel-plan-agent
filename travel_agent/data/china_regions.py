...
CHINA_DESTINATION_GROUPS: Final[list[dict[str, object]]] = [
    ...
    {"province": "山西省", "cities": ["太原市", "大同市", "阳泉市", "长治市", "晋城市", "朔州市"]},
    ...
    {"province": "浙江省", "cities": ["杭州市", "宁波市", "温州市", "嘉兴市", "湖州市", "绍兴市", "金华市", "衢州市", "舟山市", "台州市"]},
    {"province": "台湾地区", "cities": ["台北市", "新北市", "桃园市", "台中市", "台南市", "高雄市"]},
    {"province": "香港特别行政区", "cities": ["香港特别行政区"]},
    {"province": "澳门特别行政区", "cities": ["澳门特别行政区"]},
]

CHINA_DESTINATION_NAMES: Final[tuple[str, ...]] = tuple(
    city
    for group in CHINA_DESTINATION_GROUPS
    for city in group["cities"]
)
CHINA_DESTINATION_SET: Final[frozenset[str]] = frozenset(CHINA_DESTINATION_NAMES)
...
CHINA_CANONICAL_DESTINATION_SET: Final[frozenset[str]] = frozenset(
    _canonical_name(name) for name in CHINA_DESTINATION_NAMES
)


def normalize_china_destination(value: str) -> str:
    name = str(value or "").strip()
    if name in CHINA_DESTINATION_SET:
        return _canonical_name(name)
    if name in CHINA_CANONICAL_DESTINATION_SET:
        return name
    raise ValueError("目的地必须从中国城市列表中选择，不能使用自定义地点")